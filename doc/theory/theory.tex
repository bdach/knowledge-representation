...
\begin{defn}
    ...
    Każdą formułę $\gamma_i$ nazywamy komponentem formuły $\alpha$.
\end{defn}
...
Oznacza to, że po wykonaniu $A$ w stanie $\sigma$ stan wynikowy to taki, który spełnia wszystkie efekty jednego komponentu $E_i$ z~danej dekompozycji.
Wybór komponentu jest jednym z~elementów niedeterminizmu w~rozważanym systemie.
...
Aby móc zdefiniować funkcję przejścia dla~modelu, wprowadzimy kilka pojęć pomocniczych.
...
Zbiory $New$ określają, które fluenty inercjalne zmieniły~się pomiędzy dwoma stanami, a~ponadto uwzględniają możliwość dowolnych fluktuacji fluentów nieinercjalnych.
