...
\begin{align*}
    ...
    & \textsc{Peek} \Releases \textit{alive}, \textit{purring} \If \textit{alive} \\
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[->,node distance=9cm]
        ...
        \node [right of=s0,state] (s1) {$\sigma_2 = \{ \neg \textit{alive}, \neg \textit{purring} \}$};
        \node [below=of s0,initial,state] (s2) {$\sigma_1 = \{ \textit{alive}, \neg \textit{purring} \}$};

        \path (s0)  edge node [above] {$\{ \textsc{Peek} \}, \{ \textsc{Peek, Pet} \}$} (s1)
                    edge [out=90, in=180, looseness=3] node [above] {$\{ \textsc{Peek} \}, \{ \textsc{Pet} \}, \{ \textsc{Peek}, \textsc{Pet} \}$} (s0)
                    edge [out=0, in=70, looseness=0.25] node [above] {$\{ \textsc{Peek} \}, \{ \textsc{Peek, Pet} \}$} (s2);
        \path (s1)  edge [out=90, in=360, looseness=3] node [above] {$\{ \textsc{Peek} \}, \{ \textsc{Pet} \}, \{ \textsc{Peek}, \textsc{Pet} \}$} (s1);
        \path (s2)  edge [out=180, in=200, looseness=0.25] node [above] {$\{ \textsc{Peek} \}, \{ \textsc{Pet} \}, \{ \textsc{Peek}, \textsc{Pet} \}$} (s0)
                    edge [out=270, in=180, looseness=3] node [below] {$\{ \textsc{Peek} \}, \{ \textsc{Peek}, \textsc{Pet} \}$} (s2)
                    edge node [above right] {$\{ \textsc{Peek} \}, \{ \textsc{Peek, Pet} \}$} (s1);
